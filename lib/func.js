...
$(document).ready(function(){
	$("#rs_filter").on("keyup", function() {
		var value = $(this).text().toLowerCase();
		value = xoa_dau(value);
		$("#my-table-content").children().filter(function() {
...
function submitForm(){
	if($('#search_input').val() != "")
		$.ajax({
			url: 'fetchData.php',
			type: 'get',
			data: {search_content:$('#search_input').val()},
			success: function(rs){
				if(rs != ""){
					$('#my-table-content').append(rs);
					showSearchResults();
				}
				else alert('Không tìm thấy kết quả cho "'+$('#search_input').val()+'"');
			}
		});
}
